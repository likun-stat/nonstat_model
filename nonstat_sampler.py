...
if __name__ == "__main__":
   ...
   import nonstat_model.generic_samplers as sampler
   import nonstat_model.priors as priors
   import os
   import numpy as np
   ...
   for iter in np.arange(1,n_updates):
       ...
       tmp = utils.X_s_update_onetime(Y_onetime, X_onetime, X_s_onetime, cen[:,rank], cen_above[:,rank], prob_below, prob_above,
                                    ...
                                    thresh_X, thresh_X_above, R_onetime**phi, V, d, sigma_m['X_s_onetime'], random_generator)
       X_s_1t_accept = X_s_1t_accept + tmp
      
       # Update R
       Metr_R = sampler.static_metr(X_s_onetime, R_onetime**phi, utils.Rt_update_mixture_me_likelihood, 
                           priors.R_powered_prior, np.array([phi,gamma]), 2, 
                           random_generator,
                           np.nan, sigma_m['R_1t'], False, 
                           V, d)
       R_accept = R_accept + Metr_R['acc_prob']
       R_onetime = Metr_R['trace'][0,1]**(1/phi)
       ...
       if rank==0:
           ...
           # Update phi
           Metr_phi = sampler.static_metr(R, phi, utils.phi_update_mixture_me_likelihood, priors.interval_unif, 
                   hyper_params_phi, 2, 
                   random_generator,
           ...
           # Update theta_c
           Metr_theta_c = sampler.static_metr(R**phi, theta_c, utils.theta_c_update_mixture_me_likelihood, 
