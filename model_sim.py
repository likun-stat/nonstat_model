...
def phi_update_mixture_me_likelihood(data, params, Y, X_s, cen, cen_above, 
  ...
  ll = marg_transform_data_mixture_me_likelihood(Y, X, X_s, cen, cen_above, prob_below, prob_above, Loc, Scale, Shape, 
                        tau_sqd, phi, gamma, xp, surv_p, den_p) + dR_power_phi(R**phi,phi,m=0,s=gamma,log=True)
...
def X_s_likelihood_conditional(X_s, R_powered, V, d):
    # R_powered = R**phi
    tmp = norm.ppf(1-R_powered/X_s)
    if np.any(np.isnan(tmp)):
        return -np.inf
    else:
        part1 = -0.5*eig2inv_quadform_vector(V, 1/d, tmp)-0.5*np.sum(np.log(d))
        part2 = 0.5*np.sum(tmp*tmp)+np.sum(np.log(R_powered)-2*np.log(X_s))
        return part1+part2

def X_s_update_onetime(Y, X, X_s, cen, cen_above, prob_below, prob_above,
                tau_sqd, phi, gamma, Loc, Scale, Shape, xp, surv_p, den_p, 
                thresh_X, thresh_X_above, R_powered, V, d, Sigma_m, random_generator):
    ...
    for idx, X_s_idx in enumerate(X_s):
        ...
        log_num = marg_transform_data_mixture_me_likelihood_uni(Y[idx], X[idx], prop_X_s[idx], 
                       ...
                       xp, surv_p, den_p, thresh_X, thresh_X_above) + X_s_likelihood_conditional(prop_X_s, R_powered, V, d);
        log_denom = marg_transform_data_mixture_me_likelihood_uni(Y[idx], X[idx], X_s_idx, 
                       cen[idx], cen_above[idx], prob_below, prob_above, Loc[idx], Scale[idx], Shape[idx], tau_sqd, phi, gamma, 
                       xp, surv_p, den_p, thresh_X, thresh_X_above) + X_s_likelihood_conditional(X_s, R_powered, V, d);
...
def theta_c_update_mixture_me_likelihood(data, params, X_s, S, V=np.nan, d=np.nan):
  ...
  ll = np.empty(n_t)
  ll[:]=np.nan
  for idx, R_powered_i in enumerate(R_powered):
    ll[idx] = X_s_likelihood_conditional(X_s[:,idx], R_powered_i, V, d)
  return np.sum(ll)
...
def Rt_update_mixture_me_likelihood(data, params, V, d):
  X_s = data
  R_powered_t = params
  if R_powered_t < 0:
      return -np.inf
  else:
      ll = X_s_likelihood_conditional(X_s, R_powered_t, V, d)
      return ll
